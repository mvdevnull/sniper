...
if [ -z "$ALLSVHOSTSCOMMA" ] ; then
	echo "(OK) Skipping Nmap Version Scan - No appropriate hosts";
        ...
        case $yn in
				...
				sVdone=`echo `$(/usr/bin/sudo -u postgres psql -d $DB -c """SELECT count(*) from services where info = ' '"""  | grep -v row | grep -v id | grep -v """-""" )
				sVtodo=`echo `$(/usr/bin/sudo -u postgres psql -d $DB -c """SELECT count(*) from services where info = ''"""  | grep -v row | grep -v id | grep -v """-""" )
...
case $yn in
	[Yy]* ) echo "(OK) Starting - Eyewitness Scan..."
	    ...
	    $EYEWITNESS -f /tmp/sniper.eyewitness.b.txt --no-prompt --prepend-https --web --timeout 4 --threads 4 -d eyewitness;
