	...
	if [ -z "$numports" ] ; then
		...
		/bin/cp $CONF/msf_default.rc $CONF/msf.rc;
  		#Janky way to increase timeouts to exact number of "$numports".   --max-rtt-timeout "$numports"ms    --host-timeout "$numports"s
        	echo "db_nmap --top-ports="$numports" -Pn -v -n -T5 --disable-arp-ping --max-rtt-timeout "$numports"ms --version-intensity 2 --host-timeout "$numports"s --script-timeout 50s $TODOHOSTSCOMMA" >> $CONF/msf.rc ;
        	echo "quit -y" >> $CONF/msf.rc;
        	$MSFBIN -r $CONF/msf.rc;;
