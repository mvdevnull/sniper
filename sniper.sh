...
#########NMAP Probing Top-### tcp ports for known hosts============"
	...
	if [ -z "$numports" ] ; then
		...
		/bin/cp $CONF/msf_default.rc $CONF/msf.rc;
        	echo "db_nmap --top-ports="$numports" -Pn -v -n -T5 --disable-arp-ping --max-rtt-timeout 2000ms --version-intensity 2 --host-timeout 300s --script-timeout 50s $TODOHOSTSCOMMA" >> $CONF/msf.rc ;
        	echo "quit -y" >> $CONF/msf.rc;
        	$MSFBIN -r $CONF/msf.rc;;
