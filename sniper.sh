...
#########NMAP Probing Top-### tcp ports for known hosts============"
	...
	if [ -z "$numports" ] ; then
		...
		/bin/cp $CONF/msf_default.rc $CONF/msf.rc;
        	echo "db_nmap --top-ports="$numports" -Pn -n --disable-arp-ping --max-rtt-timeout 1500ms --version-intensity 2 --host-timeout 60s --script-timeout 50s --save -oG ./nmap/top-"$numports"ports-"`date +"%Y-%m-%d"`.gnmap" -oN ./nmap/top-"$numports"ports-"`date +"%Y-%m-%d"`.nmap" $TODOHOSTSCOMMA" >> $CONF/msf.rc ;
