...
#This not needed now that msfd no longer exits after sending a command... see changes from | nc localhost 55554  to msfconsole use
#SERVICE='msfd'
#if ps ax | grep -v grep | grep $SERVICE > /dev/null
#then
#        echo "(OK) - Found $SERVICE service running - skipping"
...
#Required only if using RAWR.py to thumbnail the web front pages
#SERVICE='apache2'
#if ps ax | grep -v grep | grep $SERVICE > /dev/null
#then
#        echo "(OK) - Found $SERVICE service running - skipping"
...
#This is used in case we have some Sat-Scanner results
if ls $CWD/../misc/sat-scanner/NmapScans/*.* &> /dev/null; then 
	echo "(OK) - SatScanner Nmap Files Found"
	cp $CWD/../misc/sat-scanner/NmapScans/*.xml $RESULTS/all
fi
...
cd $RESULTS
NEWFILES="$(/usr/bin/diff ./all ./import_complete/ | grep "Only in ./all" | cut -d ":" -f 2)"

for i in "$NEWFILES"
	do
		#Possibly don't copy if $NEWFILES is null
		if [ -z "$NEWFILES" ]
			then
				continue
			else
				echo "(OK) - New Nmap file $i will be imported into MSF database" 
				i=`echo "$i" | sed 's/^ *//g'`
				cp $RESULTS/all/$i $RESULTS/new/
		fi
	done
cd $CWD
...
if ls $RESULTS/new/*.* &> /dev/null; then
		...
		#echo "db_import $RESULTS/new/*.xml" | nc 127.0.0.1 55554
...
if [ $TOTALHOSTS -eq "0" ] ; then
	echo "(Not OK!!) You have *NO* hosts in the database $DB. Enter the IP Range for QUICK host discovery? [Nmap compatible example - 192.168.1.1-200]"
    	read IPRANGE
	echo "============Phase 1.a - Nmap -Pn -n -v --disable-arp-ping -p 22,80,443,445 Scan ()=========="
	#echo "db_nmap -Pn -n -v --disable-arp-ping -p 22,80,443,445 $IPRANGE " | nc 127.0.0.1 55554 
	/bin/cp $CONF/msf_default.rc $CONF/msf.rc
...
#########Subsequent discovery scan ===================="
        ...
        case $yn in
                ...
                #echo "db_nmap -Pn -n --top-ports=200 $TODOHOSTSCOMMA" | nc 127.0.0.1 55554 ;
                #echo "";;
                /bin/cp $CONF/msf_default.rc $CONF/msf.rc;
...
echo "===========Phase 1 - General NMAP Scan==========="
...
#Maybe something like this need to test though -- nah, probably not.. something else.. but for later.. 
#SELECT DISTINCT H.address from hosts H, services S where S.state in ('open','closed') and S.host_id = H.id and S.info is NULL
...
#########NMAP Probing Top-200 tcp ports for known hosts============"
	echo "Hosts missing ports outside of 22,80,443,445.  These may need to be port scanned: "
	echo "$TODOHOSTS"
	...
	TODOHOSTSCOMMA=$(echo "$TODOHOSTSCOMMA" | sed 's/,/, /g')
        read -p "(?) Do you want nmap to perform a (--top-ports=200 ) syn scan ?(y/N)" yn
...
######################################################################
#RAWR#####
#We might need to add the "SAVE" option to do this rawr.... but for now it seems to crash if you add the "save" 
#echo "db_nmap -Pn -n --top-ports=200 $TODOHOSTSCOMMA save" | nc 127.0.0.1 55554 ;;
#if ls /root/.msf4/local/*.xml &> /dev/null; then
#	cp /root/.msf4/local/*.xml $RESULTS/import_complete/latest-tcp.xml
#	#rm -rf /root/.msf4/local/*.xml
#	read -p "(?) Do you want to thumbnail all web pages with 'rawr' ?(y/N)" yn

#	case $yn in
#        	[Yy]* ) echo "============Phase 1.c - rawr  -f $CWD/results/import_complete/latest-tcp.xml -d /var/www/rawr -e -q -o -r ()=========="
#        		$CWD/third-party-tools/rawr/rawr.py -f $RESULTS/import_complete/latest-tcp.xml -d /var/www/rawr -e -q -o -r;;
#
#        	[Nn]* ) echo "(OK) Skipping rawr.py ";;
#        	* ) echo "(OK) Skipping rawr.py ";;
#
#	esac
#
#else
...
cd $CWD
echo "Hosts (with empty banner ports) that can be nmap -sV scanned: "
...
echo  "$ALLSVHOSTSCOMMA"
        ...
        case $yn in
		[Yy]* ) echo "============Phase 1.c - Nmap (-sV -n) Scan ()==========";
                	echo "(OK) - About to run db_Nmap -sV port scan to $DB database - (all Nmap scan output to $DB...)";
                	# Begin Outer Loop
                        for i in $ALLSVHOSTS
                        do  
                                ...
                                #echo "db_nmap -sV -n $i -p $DBNMAPCOMMA " | nc -w 1 127.0.0.1 55554 
				/bin/cp $CONF/msf_default.rc $CONF/msf.rc
                		echo "db_nmap -sV -n $i -p $DBNMAPCOMMA " >> $CONF/msf.rc
                		echo "quit -y" >> $CONF/msf.rc
                		$MSFBIN -r $CONF/msf.rc
				...
				echo "BUG HERE!!!! - if no blanks, SQL error.   Also, why does example 172....254  miss port 139, but only update 0 not 1?"
				#Something like this should have worked, but didn't.. keep in mind.. UPDATE services set info = ' ' where id in  (SELECT host_id from services where host_id = (select id from hosts where address = '$i') and info = '')
...
echo "==========Phase 2 - General NESSUSCMD & MSF Scans ================"
cd /tmp
...
echo "$UNIXSSHCOMMA"
	read -p "(?) Do you want to provide NESSUSCMD with SSH credentials to definitively determine the OS name and flavor?(y/N)" yn
	case $yn in
...
echo "$WINWMICOMMA"
	read -p "(?) Do you want to provide NESSUSCMD with WINDOWS credentials to definitively determine the OS name and flavor?(y/N)" yn
...
##########################WINWMI auxiliary MSF a)smb_version, b)nbname(for hostname) and c)endpoint_mapper (for other hostname when nbname doesn't work)
        ...
        case $yn in
                [Yy]* ) echo "==========Phase 2.c Starting - MSF auxiliary scan a)SMB Version b)nbname and c)endpoint_mapper Windows OS Probes==========";
			...
			echo "hosts -S Windows -R" >> $CONF/msf.rc;
...
#################BEGIN BETA NESSUSCMD/MSF######################
#read -p "(**BETA**) Generate NESSUSCMD/Metasploit Correlated (WMI) Scan?(y/N)" yn
#        case $yn in
#                [Yy]* ) NSSMSF="$(cd /opt/nessus/lib/nessus/plugins; /bin/grep -f $CWD/conf/msf_whitelist1.txt *.nasl | cut -d """:""" -f1 | uniq | xargs grep -f $CWD/conf/msf_whitelist.txt | grep -v -i -f $CWD/conf/msf_blacklist.txt | grep -B1 """metasploit_name""" | cut -d """(""" -f2 | cut -d """:""" -f3 | cut -d """)""" -f1 | cut -d """'""" -f2 | cut -d """'""" -f1 | grep -v """\-\-"""| grep -e ^[0-9][0-9][0-9][0-9][0-9])";
#                        for i in $NSSMSF; do NSSMSFCOMMA=`echo $NSSMSFCOMMA$i\,`; done;
#                        NSSMSFCOMMA=$(echo "$NSSMSFCOMMA" | sed '$s/.$//');
...
#        esac
#
#echo "**BETA** - NESSUSCMD/Metasploit Correlations (WINDOWS)- see ./results/nss-msf-win.txt"
